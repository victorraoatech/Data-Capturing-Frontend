...
export default function AdminSignupPage() {
  ...
  const { mutate: submitMutate, isPending } = useMutation({
    mutationFn: async (values: { 
      ...
      const payload = {
        firstName: firstName,
        lastName: lastName,
        name: `${firstName} ${lastName} Organization`, // Organization name
        email: values.email.toLowerCase().trim(),
        phone: values.phone.trim(),
        password: values.password,
        subdomain: `${firstName.toLowerCase()}${lastName.toLowerCase()}${Date.now()}` // Generate unique subdomain
      };
      
      console.log('🚀 Sending ADMIN registration to backend:', payload);

      const { data } = await client.post('https://datacapture-backend.onrender.com/api/organisations/register', payload);
      return data;
    },
    onSuccess: (data) => {
      toast({ 
  ...
  const validate = (): boolean => {
    ...
    if (!formValues.subdomain.trim()) newErrors.subdomain = 'Subdomain is required';
    else if (!/^[a-zA-Z0-9-]+$/.test(formValues.subdomain)) newErrors.subdomain = 'Subdomain can only contain letters, numbers, and hyphens';
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="relative" style={{ width: "1440px", minHeight: "1400px", margin: "0 auto" }}>
        {/* Left Image Section */}
        <div 
          className="absolute"
          style={{
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              <div className={`input-container ${formValues.organizationName ? 'has-value' : ''} ${errors.organizationName ? 'error' : ''}`}>
                <input
                  type="text"
                  name="organizationName"
                  value={formValues.organizationName}
                  onChange={handleChange}
                  placeholder="Pavelet Sportswear"
                />
            ...
            >
              <div className={`input-container ${formValues.subdomain ? 'has-value' : ''} ${errors.subdomain ? 'error' : ''}`}>
                <input
                  type="text"
                  name="subdomain"
