...
export default function UserPage(): JSX.Element {
  ...
  //     const { data } = await client.post("/api/v1/auth/user/register", payload);
  //     return data;
  //   },
  //   onSuccess: () => {
      ...
      <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
        {() => (
          <Form>
            ...
            <div className="mb-4">
              <label className="block text-gray-700 font-medium mb-1">Other Name</label>
              <Field 
                type="text" 
                name="othername" 
                placeholder="Input text" 
                className="w-full px-3 py-2 border border-gray-300 rounded-md" 
              />
              <ErrorMessage name="othername" component="div" className="text-red-500 text-sm mt-1" />
            </div>
            ...
            <button 
              type="submit" 
              className="w-full bg-[#EAAB40] text-white rounded-md py-2 mb-4 hover:bg-yellow-600" 
              // disabled={isPending}
            >
              Sign Up
